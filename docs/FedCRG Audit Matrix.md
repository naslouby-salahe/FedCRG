...
| K06 | Statistics (bootstrap replicates, seeds, familywise alpha, margins) owned by YAML | `configs/statistics/confirmatory.yaml` | `config/statistics_config.py` | YAML | present | — | StatisticsConfig + YAML created; threaded through bootstrap/contrasts/margins/comparators | config-resolved; no hidden defaults | VERIFIED |
...
| M08 | ~~Claim gates G0-G8 and claim levels~~ REMOVED 2026-08-13 at user request: `analysis/claim_gates.py`, `ClaimLevel` enum, `fedcrg claims evaluate`, and the unused utility-margin helpers (`utility_anchor`/`utility_preserved`/`assess_utility`/`UtilityAssessment`) deleted | n/a | n/a | n/a | absent | — | out of scope per maintainer | none | REMOVED |
