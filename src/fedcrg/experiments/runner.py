    ...
    def write_policy_artifacts(
        ...
        threshold_records = [
            ThresholdRecord(
                ...
                cp_upper=protocol_by_client[row.client_id].mismatch.interval.upper,
                p_low=protocol_by_client[row.client_id].mismatch.p_low,
                p_high=protocol_by_client[row.client_id].mismatch.p_high,
                state=protocol_by_client[row.client_id].decision.state,
                tie_count=protocol_by_client[row.client_id].readiness.tie_count,
        ...
        atomic_write_json(layout.admission, summarize_admission(bundle.protocol_results))
...
class EvaluationSummary(BaseModel):
    ...
    calibration_seed: CalibrationSeed
    calibration_assignment: CalibrationAssignmentMode
...
class PolicyCellMaterializer:
    ...
    def _copy_manifests(self, layout: RunLayout, caches: FrozenCacheInputs) -> None:
        prepared_layout = PreparedDatasetLayout(caches.prepared_root)
