...
from fedcrg.datasets.prepare import ClientData, DatasetAdapter, DatasetDiscovery
from fedcrg.datasets.splits import stable_row_id
...
class DiadAdapter(DatasetAdapter):
    ...
    def load_client(self, client_id: ClientId) -> ClientData:
        ...
        for path in self.source_files():
            ...
            for chunk in pd.read_csv(shard, chunksize=self.chunk_size):
                ...
                benign_parts.append(
                    model.loc[benign_mask].drop(columns=["attack_group"], errors="ignore")
                )
    ...
    def _attack_category_column(frame: pd.DataFrame, anomaly_column: str) -> str:
        column = next(
            (
                name
                for name in _ATTACK_CATEGORY_COLUMNS
                if name in frame.columns and name != anomaly_column
            ),
            None,
        )
        if column is None:
