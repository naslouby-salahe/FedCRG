"""Benign-only personalization ablations."""
...
def readiness_only(client: ClientPolicyData) -> float:
    readiness = client.protocol.readiness
    if readiness.plan.state is CalibrationReadinessState.READY and readiness.tie_count == 1 and readiness.threshold is not None:
        return readiness.threshold
    return client.protocol.reference.value

def mismatch_only(client: ClientPolicyData, alpha: float) -> float:
