...
def oracle_choice(client: ClientPolicyData, candidates: tuple[float, float, float], band: OperatingBand) -> float:
    ...
    for order, threshold in enumerate(candidates):
        ...
        ranked.append((band_error(client_fpr, band), -tpr(attack_cm), order, threshold))
    return min(ranked)[3]
