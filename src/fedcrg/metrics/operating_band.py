...
def high_excess(fpr_value: float, band: OperatingBand) -> float:
    return max(0.0, fpr_value - band.upper)


def band_violation(fpr_value: float, band: OperatingBand) -> float:
    return float(not band.contains(fpr_value))


def absolute_fpr_error(fpr_value: float, alpha: float) -> float:
