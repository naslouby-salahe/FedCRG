...
_FAMILY_MARKERS: dict[NbaiotAttackFamily, tuple[NbaiotAttackFamily | NbaiotFileMarker, ...]] = {
    NbaiotAttackFamily.MIRAI: (NbaiotAttackFamily.MIRAI,),
    NbaiotAttackFamily.GAFGYT: (NbaiotAttackFamily.GAFGYT, NbaiotFileMarker.BASHLITE),
}
...
class NBaiotAdapter(DatasetAdapter):
    def __init__(self, root: Path, dataset: DatasetConfig) -> None:
        ...
        if dataset.id is not DatasetId.NBAIOT:
            raise DataIntegrityError("The N-BaIoT adapter requires the N-BaIoT dataset contract")
        if (
            not dataset.feature_names
            or not dataset.source_headers
            or not dataset.device_directories
        ):
            raise DataIntegrityError(
                "The N-BaIoT contract must declare its feature names, source headers, and devices"
            )
    ...
    def _map_directories(self) -> dict[ClientId, Path]:
        ...
        for device in self.dataset.device_directories:
            matches = [
                directory
                for directory in directories
                if all(token in _normalized_name(directory) for token in device.tokens)
            ]
            if len(matches) != 1:
                raise DataIntegrityError(
