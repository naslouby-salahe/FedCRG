...
def _load(relative: str):
    config = ExperimentConfigResolver().resolve(ROOT / relative)
    validate_experiment_config(config)
    return config


def test_primary_profile_is_frozen() -> None:
    config = _load("configs/experiments/primary/nbaiot.yaml")
...
def test_second_detector_profile_is_frozen() -> None:
    config = _load("configs/experiments/robustness/deep_svdd_nbaiot.yaml")
