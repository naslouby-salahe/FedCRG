...
def test_client_fraction_is_locked_to_full_participation() -> None:
    ...
    config = TrainingConfig(rounds=1, local_epochs=1, batch_size=2, learning_rate_initial=1e-3, learning_rate_final=1e-3)
...
def test_client_fraction_below_one_is_rejected() -> None:
    with pytest.raises(ValidationError):
        TrainingConfig(rounds=1, local_epochs=1, batch_size=2, client_fraction=0.5, learning_rate_initial=1e-3, learning_rate_final=1e-3)
